...
const buildGraphqlWsUrl = (baseUrl: string, provided: string | undefined) => {
  ...
  try {
    ...
    throw new Error(`Invalid base URL provided for GraphQL websocket endpoint: ${baseUrl}`);
  }
};
...
const collectConfig = (): AppConfig => {
  ...
  const rawGraphqlUrl = process.env.NEXT_PUBLIC_GRAPHQL_URL ?? process.env.GRAPHQL_URL;
  const rawGraphqlWsUrl = process.env.NEXT_PUBLIC_GRAPHQL_WS_URL ?? process.env.GRAPHQL_WS_URL;
