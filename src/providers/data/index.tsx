...
const isUserNotFoundError = (error: unknown) => {
  ...
  const maybeResponseErrors = (error as { response?: { errors?: Array<{ message?: string }> } }).response?.errors;
  if (maybeResponseErrors && maybeResponseErrors.some((item) => item?.message?.includes("Unable to find UserEntity with id"))) {
    return true;
  }
...
export const wsClient = shouldCreateWsClient
  ? createClient({
      ...
      connectionParams: async () => {
        ...
        const maybeUser = session?.user as (Session["user"] & {
          accessToken?: string;
        }) | undefined;
